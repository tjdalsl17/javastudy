...
public class ContactServiceImpl implements ContactService {
	...
	public void removeContact() {
		...
		List<ContactDTO> contactList = dao.selectContactsByNameList(name);   // 트랜잭션 처리x
	...
	public void modifyContact() {
		...
		List<ContactDTO> contactList = dao.selectContactsByNameList(name);
		System.out.println("<<< 조회된 연락처 목록 >>>");
		for(ContactDTO contact : contactList) {
			System.out.println(contact);
		}
		...
		switch(choice) {
		case "1":
			System.out.print("수정할 이름 입력 >>> "); // 선택한 항목만 바꿔준다.
